...
export function GameSettings({ startLife, historyDelay, closeRadialOnDialog, showTime, darkMode, showFloatingNumbers, holdIncrement, soundEnabled, soundVolume, onStartLifeChange, onHistoryDelayChange, onCloseRadialOnDialogChange, onShowTimeChange, onDarkModeChange, onShowFloatingNumbersChange, onHoldIncrementChange, onSoundEnabledChange, onSoundVolumeChange }: { startLife: number; historyDelay: number; closeRadialOnDialog: boolean; showTime: boolean; darkMode: boolean; showFloatingNumbers: boolean; holdIncrement: number; soundEnabled: boolean; soundVolume: number; onStartLifeChange: (value: number) => void; onHistoryDelayChange: (value: number) => void; onCloseRadialOnDialogChange: (value: boolean) => void; onShowTimeChange: (value: boolean) => void; onDarkModeChange: (value: boolean) => void; onShowFloatingNumbersChange: (value: boolean) => void; onHoldIncrementChange: (value: number) => void; onSoundEnabledChange: (value: boolean) => void; onSoundVolumeChange: (value: number) => void }) {
  ...
  return (
    <div className="settings-content">
      ...
      <div className="number-row"><span className="number-label">Hold increment</span><div className="number-controls"><button className="number-btn" onClick={() => onHoldIncrementChange(Math.max(1, holdIncrement - 1))} aria-label="Decrease">−</button><span className="number-value">{holdIncrement}</span><button className="number-btn" onClick={() => onHoldIncrementChange(Math.min(50, holdIncrement + 1))} aria-label="Increase">+</button></div></div>
      ...
      <div className="toggle-row" style={{ justifyContent: 'space-between' }}><span className="toggle-label">Show counter tracker</span><button className="checkbox-toggle" onClick={() => onShowFloatingNumbersChange(!showFloatingNumbers)} aria-label="Toggle counter tracker" style={{ borderColor: showFloatingNumbers ? '#4652f5' : '#d9d7df', backgroundColor: showFloatingNumbers ? '#4652f5' : 'transparent' }}>{showFloatingNumbers && <Check size={16} color="#fff" strokeWidth={3} />}</button></div>
      <div className="toggle-row" style={{ justifyContent: 'space-between' }}><span className="toggle-label">Sound effects</span><button className="checkbox-toggle" onClick={() => onSoundEnabledChange(!soundEnabled)} aria-label="Toggle sound effects" style={{ borderColor: soundEnabled ? '#4652f5' : '#d9d7df', backgroundColor: soundEnabled ? '#4652f5' : 'transparent' }}>{soundEnabled && <Check size={16} color="#fff" strokeWidth={3} />}</button></div>
      {soundEnabled && (
        <div style={{ marginTop: '12px', padding: '12px', backgroundColor: 'var(--line)', borderRadius: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '14px', color: 'var(--muted)', minWidth: '60px' }}>Volume</span>
            <input type="range" min="0" max="1" step="0.1" value={soundVolume} onChange={(e) => onSoundVolumeChange(parseFloat(e.target.value))} style={{ flex: 1, cursor: 'pointer' }} aria-label="Sound volume" />
            <span style={{ fontSize: '14px', color: 'var(--muted)', minWidth: '40px', textAlign: 'right' }}>{Math.round(soundVolume * 100)}%</span>
          </div>
        </div>
      )}
